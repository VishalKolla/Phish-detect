...
def generate_external_dataset(url):
    ...
    if res == "Unaccessable":
        return "Unaccessable URL. Please check if site is online and URL is inputted properly"
    if not res:
    ...
    with open('adaboost_model.pkl', 'rb') as f:
        ada_clf = joblib.load(f)

    value = ada_clf.predict(df)

    if value == "phishing":
        return "This URL is probably a phishing website"
    else:
        return "This URL is probably legitimate"
...
if st.button("Check"):
    if url_input:
        ...
        prediction = generate_external_dataset(url_input)
        st.write("Prediction:", prediction)
    else:
        st.write("Please enter a URL.")
st.write("Current test accuracy: 0.9667541557305337")
temp_val = [[1118, 37], [39, 1092]]
class_names = ["Phishing", "Legitimate"]
cm = np.array(temp_val)
fig, ax = plt.subplots()
sns.heatmap(cm, annot=True, fmt='d', cmap = sns.cubehelix_palette(start=.23, rot=-.5, light = 0.65, reverse = True, as_cmap=True), cbar=False, ax=ax, xticklabels=class_names, yticklabels=class_names)
plt.xlabel('Predicted Label')
plt.ylabel('True Label')
st.pyplot(fig)
st.subheader("Citations:")
st.write("Hannousse, Abdelhakim; Yahiouche, Salima (2021), “Web page phishing detection”, Mendeley Data, V3, doi: 10.17632/c2gw7fy2j4.3")
